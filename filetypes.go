...
const (
	...
	ZIP
	GZIP
	LZW // compressed with Lempel-Ziv-Welch algorithm
	LZH // compressed with LZH
	BZ2 // compressed with Bzip2
	EXE // DOS MZ executable file format and its descendants
)
...
func init() {
	...
	typeTree.Insert(LZW, []byte{0x1F, 0x9D})
	typeTree.Insert(LZH, []byte{0x1F, 0xA0})
	typeTree.Insert(BZ2, []byte("BZh"))
	typeTree.Insert(EXE, []byte("MZ"))
}
